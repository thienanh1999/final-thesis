...
export default class ProjectMemberModal extends React.Component<IProjectMemberModalProps, IProjectMemberModalState> {
	...
	componentDidMount() {
		...
		userAPI
			...
			.then(res => {
				...
				if (res && res.data && res.status === 200 && res.data.results && Array.isArray(res.data.results) && res.data.results.length > 0) {
					const notPrjMembers: UserBasicInfo[] = [];
					let ok: boolean = false;
					res.data.results.forEach((fetchedUser: any) => {
						ok = true;
	...
	private customList = (items: UserBasicInfo[], isLeft: boolean) => (
		<Paper sx={{ width: 400, height: 300, overflow: 'auto' }}>
			...
			<List sx={{pt:0,pb:0}} dense component="div" role="list">
				{items.map((userInfo: UserBasicInfo) => {
					...
					return (
						...
						>
							...
							<ListItemText
								id={labelId}
								primary={`${userInfo.name}${this.props.ownerId === userInfo.id ? " (Quản lý dự án)" : ""}`}
		...
		return !loading ?
			<Box sx={{width: "920px"}}>
				...
				<Button
					...
					sx={{ mt: 2, mr: 2, background: "#BC6181" }}>
					Hủy bỏ
				</Button>
				<Button
					variant="contained"
